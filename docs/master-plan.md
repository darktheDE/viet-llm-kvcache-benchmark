...
### PHASE 1: KHỞI ĐỘNG, SETUP MÔI TRƯỜNG & CHUẨN BỊ DỮ LIỆU (Tuần 1 - Tuần 2)
*   **Mục tiêu:** Thống nhất đề cương, cài đặt môi trường Cloud GPU, chuẩn bị và làm sạch dữ liệu.
*   **Nhiệm vụ chi tiết:**
    *   *Quản lý & Điều phối (Kiến Hưng, Việt Anh):* Thiết lập không gian làm việc chung (Drive, Notion, Plane.so), tạo template biên bản họp `kickoff/minutes_template.md`.
    *   *Nghiên cứu (Quốc Anh, Trọng Phú):* Hoàn thiện đề cương chi tiết bằng tiếng Anh, viết định nghĩa bài toán (`problem_definition.md`).
    *   *Kỹ thuật (Việt Anh, Minh Quân, Minh Khánh, Quang Duy):* Tạo tài khoản trên nền tảng Cloud GPU (Vast.ai hoặc RunPod); thiết lập môi trường Conda với CUDA 12.x, PyTorch và vLLM; cấu hình cấu trúc thư mục chuẩn của Git.
    *   *Dữ liệu (Hồ Phát, Hữu Huy, Ngọc Thạch):* Tải các bộ dữ liệu `VMLU`, `VTSNLP`; áp dụng công cụ `NVIDIA NeMo Curator` để chuẩn bị dữ liệu tiếng Việt sạch; thiết kế và đóng gói bộ test-set nhỏ (~10-20 mẫu ngữ cảnh dài) tại file `datasets/test_set_small.json`.

### PHASE 2A: THIẾT LẬP PIPELINE CHUẨN & CHẠY THỬ baseline (Tuần 3)
*   **Mục tiêu:** Xây dựng script đo đạc tự động và chạy kiểm thử thành công trên 1 mô hình nền tảng BF16 (Full KV Cache).
*   **Nhiệm vụ chi tiết:**
    *   *Kỹ thuật (Minh Quân, Minh Khánh, Quang Duy):* Hoàn thiện script đo đạc `scripts/run_baseline.py` ghi nhận tự động các chỉ số: Peak VRAM (Prefill/Decode), TTFT, ITL, Throughput và Perplexity.
...
*   **Mục tiêu:** Thu thập toàn bộ số liệu thực nghiệm trên 4 dòng mô hình mục tiêu.
*   **Nhiệm vụ chi tiết:**
    *   *Kỹ thuật & Thử nghiệm (Nhóm phân vai 1-2 mô hình/người):*
        ...
        *   **Hỗ trợ chung (Duy - Infrastructure):** Đảm nhận vai trò xử lý lỗi (bug-fixing), tối ưu hóa bộ nhớ và giám sát việc chạy thử nghiệm các mốc nén KV cache: FP8, HQQ, PolarQuant, TurboQuant (mốc Full) và TurboQuant (mốc tắt QJL).
    *   *Đo đạc & Ghi kết quả (Toàn bộ team kỹ thuật):* Ghi nhận số liệu chính xác vào các file kết quả CSV riêng biệt của từng mô hình theo đúng template chuẩn.
...
*   **Nhiệm vụ chi tiết:**
    *   *Phân tích (Ngọc Thạch - Phân tích chính, Hồ Phát, Hữu Huy):* Viết script Python `scripts/plot_results.py` sử dụng pandas và matplotlib/plotly để đọc dữ liệu từ các file CSV; tổng hợp trung bình/độ lệch chuẩn.
    *   *Trực quan hóa (Ngọc Thạch):* Xuất ra các biểu đồ trực quan lưu vào thư mục `results/plots/`:
        *   Biểu đồ 1: Memory (VRAM) vs. Context Length (màu sắc biểu thị phương pháp nén).
        *   Biểu đồ 2: Latency/Throughput vs. Context Length.
        *   Biểu đồ 3: Perplexity vs. Compression Methods (Trade-off Pareto curves).
    *   *Nghiên cứu (Quốc Anh, Trọng Phú):* Phân tích các mô hình suy giảm chất lượng, rút ra nhận xét cốt lõi về sự nhạy cảm của tiếng Việt khi nén sâu.

### PHASE 4: HOÀN THIỆN VĂN BẢN, VIẾT BÁO CÁO & PAPER (Tuần 7)
*   **Mục tiêu:** Hoàn thiện 100% tài liệu bàn giao, chạy thử slide thuyết trình và chuẩn bị bảo vệ.
*   **Nhiệm vụ chi tiết:**
...
|---|---|:---:|---|
| **Rủi ro Phần cứng** | Server local không có GPU hoặc GPU yếu, không chạy được nhân Triton/CUDA của vLLM. | **Cao** | Sử dụng ngân sách nhóm thuê Cloud GPU (Vast.ai, RunPod). Chi phí dự kiến cực rẻ (~250.000 VNĐ cho cả dự án). |
