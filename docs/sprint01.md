...
*   **Bước 4:** Chạy kiểm thử đo đạc thực tế mốc không nén (Full KV Cache BF16) với mô hình `PhoGPT-7B5-Instruct` sử dụng bộ dữ liệu `datasets/test_set_small.json`. Ghi nhận thử nghiệm kết quả đầu ra vào file CSV cục bộ.
