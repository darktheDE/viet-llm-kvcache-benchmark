...
*   [x] File CSV ghi nhận chính xác các chỉ số đo được (Thay thế base/dynamic VRAM và compression_ratio bằng Peak VRAM động đo qua pynvml background sampler + các cờ phân tích chất lượng ngôn ngữ).

> [!NOTE]
> **Tình trạng Triển khai & Thay đổi thiết kế:**
> * **Đã triển khai:** Tự động đo Peak VRAM chính xác nhất bằng luồng chạy nền `VRAMMonitor` (sử dụng `pynvml` lấy mẫu mỗi 50ms) thay cho việc đọc VRAM thụ động sau sinh từ.
> * **Thay đổi thiết kế:** Các cột `compression_ratio`, `gpu_efficiency_index`, `base_vram_mb`, `dynamic_vram_mb` được lược bỏ vì vLLM mặc định chiếm dụng trước 85% VRAM tĩnh làm Block Pool (PagedAttention), khiến VRAM nền và VRAM động gần như trùng khớp. Thay vào đó, nhóm chuyển sang đo đạc Peak VRAM và các chỉ số suy thoái ngôn ngữ chuyên sâu.
...
#### 5. Tiêu chuẩn hoàn thành (Definition of Done - DoD)
*   [/] Hoàn thành các lượt chạy thành công cho các dòng mô hình mục tiêu ở các mốc ngữ cảnh trước khi xảy ra OOM (Đã chạy xong 3/4 mô hình chính là Qwen3, Qwen2.5, Mistral; đang thiếu Llama 3.1).
*   [x] Số liệu trong file CSV không bị trống (null) ở các trường thông số phần cứng của những cấu hình đã chạy.

> [!NOTE]
> **Tình trạng Triển khai & Còn thiếu:**
> * **Đã triển khai:** Đã chạy xong 45 cấu hình của Qwen3, Qwen2.5, và Mistral (4k/8k). Bổ sung chạy grid extra cho Phi-4 (9 cấu hình) và Gemma-3 (3 cấu hình).
> * **Còn thiếu:** Chưa chạy Grid Search cho mô hình `llama3.1:8b-instruct-fp16` (thiếu 15 cấu hình).
...
#### 5. Tiêu chuẩn hoàn thành (Definition of Done - DoD)
*   [x] Chỉ số Perplexity được ghi nhận đầy đủ cho các mốc chạy thử nghiệm thông qua script offline mà không gây lỗi crash runtime.
*   [x] File CSV dữ liệu kết quả đo đạc sạch sẽ, không bị lệch cột.

> [!NOTE]
> **Tình trạng Triển khai & Thay đổi thiết kế:**
> * **Đã triển khai:** Viết bộ script `compute_all_ppl.py` chạy offline đo Perplexity khách quan bằng mô hình BF16 gốc tham chiếu, tính toán các chỉ số chất lượng ngôn ngữ khác như `repeated_ngram_ratio`, `special_char_ratio` và gán nhãn cảnh báo lặp từ `repetition` / `gibberish`.
> * **Thay đổi thiết kế:** Bỏ qua các cột VRAM phụ để đồng nhất theo schema dữ liệu thực tế của vLLM.
> * **Còn thiếu:** Chưa tính được PPL cho Gemma-3 4B do tài khoản Hugging Face chưa được duyệt quyền truy cập gated model `google/gemma-3-4b-it` trên server chạy offline.
...
*   **Sprint:** Sprint 2 (Week 3-4)
*   **Team / Role:** Research (Nguyen Dang Quoc Anh) / Writing & Coordination (Phan Trong Phu - Paper Draft Support)
*   **Priority:** Medium
...
#### 5. Tiêu chuẩn hoàn thành (Definition of Done - DoD)
*   [x] Phần chương Methodology viết bằng tiếng Anh học thuật hoàn chỉnh trên hệ thống Overleaf.
*   [x] Toàn bộ các tài liệu tham khảo chính yếu về TurboQuant, PolarQuant đã được liên kết chính xác trong tệp `.bib` và trích dẫn thành công trong văn bản.

> [!NOTE]
> **Tình trạng Triển khai:**
> * Đã hoàn thành cấu hình dự án Overleaf chung sử dụng template chuẩn IEEE Conference 2 cột. Soạn thảo chi tiết chương Methodology mô tả toán học của TurboQuant (Lloyd-Max quantization, random projection, QJL error-correction) và PolarQuant. Cập nhật danh mục bibtex chứa đầy đủ references.
