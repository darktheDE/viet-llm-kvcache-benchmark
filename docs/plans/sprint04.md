...
*   **Module:** Phase 4: Viết bài + chỉnh sửa
*   **Sprint:** Sprint 4 (Week 7)
*   **Team / Role:** Research (Nguyen Dang Quoc Anh) / Writing & Coordination (Phan Trong Phu - Paper Editing Support)
*   **Priority:** High
...
*   **Sprint:** Sprint 4 (Week 7)
*   **Team / Role:** Technical & Experiment (Pham Minh Quan - Lead, Tran Minh Khanh, Huynh Ngoc Thach) / Writing & Coordination (Nguyen Van Quang Duy - Documentation Liaison)
*   **Priority:** High
...
#### 5. Tiêu chuẩn hoàn thành (Definition of Done - DoD)
*   [x] Mã nguồn trên GitHub được tổ chức gọn gàng, không có file thừa hoặc file log rác được đẩy lên.
*   [x] Thực hiện cài đặt thử nghiệm môi trường từ đầu trên một máy ảo mới bằng lệnh `pip install -r requirements.txt` hoạt động thành công.
*   [x] Tài liệu `README.md` hiển thị đầy đủ, không bị lỗi định dạng markdown, mô tả chi tiết cách thức chạy lại các lệnh benchmark để tái lập kết quả.
...
### TASK 5: TRIỂN KHAI PIPELINE TỐI ƯU HÓA KHỞI CHẠY & ĐÁNH GIÁ CHẤT LƯỢNG SINH CHUYÊN SÂU (TECHNICAL & EXPERIMENT / DATA & ANALYSIS)

*   **Title:** `[TECH/DATA-ADD] Xây dựng script tối ưu load model Mistral, bộ tính toán Perplexity Offline & Điền chỉ số chất lượng văn bản sinh`
*   **Module:** Phase 2 & Phase 3: Thực nghiệm nâng cao + Đánh giá chất lượng
*   **Sprint:** Sprint 4 (Week 7)
*   **Team / Role:** Technical & Experiment (Pham Minh Quan - Lead) / Data & Analysis (Nguyen Ho Phat)
*   **Priority:** High

#### 1. Mô tả chi tiết Task (Description)
Thiết lập phương pháp tối ưu hóa việc nạp model Mistral vào GPU một lần cho tất cả context lengths, giảm thời gian chết của GPU. Triển khai script tính toán Perplexity (PPL) offline bằng mô hình tham chiếu gốc không nén để tránh bias và đo đạc các chỉ số chất lượng sinh nâng cao nhằm phát hiện sớm hiện tượng suy giảm chất lượng (lặp từ, sinh văn bản rác).

#### 2. Tài liệu đọc tham khảo (References)
*   *Tải model HuggingFace:* `transformers` and `torch` APIs.
*   *Đặc tả Perplexity:* `docs/plans/PPL.md` - Tài liệu phương án đo đạc Perplexity.

#### 3. Từng bước thực hiện chi tiết (Step-by-Step)
*   **Bước 1:** Viết script `run_mistral_optimized.py` và `run_mistral_single_method.py` tích hợp cơ chế nạp model một lần duy nhất cho mỗi phương pháp nén.
*   **Bước 2:** Viết script `compute_all_ppl.py` và `compute_ppl_offline.py` để tính PPL offline dựa trên các tệp sinh ra từ benchmark và ghi ngược lại vào CSV.
*   **Bước 3:** Bổ sung các chỉ số kiểm tra độ lặp từ (`repeated_ngram_ratio`) và kiểm tra ký tự rác (`special_char_ratio`) để thiết lập cờ cảnh báo chất lượng (`quality_warning`).
*   **Bước 4:** Khắc phục lỗi tràn ngữ cảnh 16k của Mistral bằng cách cập nhật hệ số phình to token lên `1.9` và clamp `32768`.

#### 4. Kết quả đầu ra (Expected Output)
*   Mã nguồn các script tối ưu và PPL offline được đẩy lên Git.
*   Dữ liệu kết quả cuối cùng chứa đầy đủ Perplexity và cờ cảnh báo chất lượng sinh (`template_log_real_run_all.csv`, `template_log_real_run_mistral_final_all.csv`).

#### 5. Tiêu chuẩn hoàn thành (Definition of Done - DoD)
*   [x] Script chạy tối ưu hóa Mistral hoạt động ổn định và ghi nhận đúng kết quả.
*   [x] Script tính PPL offline tự động backfill đầy đủ dữ liệu cho các file kết quả mà không gây lỗi lệch cột.
*   [x] Xác định và xử lý triệt để nguyên nhân lỗi tràn ngữ cảnh 16k của Mistral 7B.
